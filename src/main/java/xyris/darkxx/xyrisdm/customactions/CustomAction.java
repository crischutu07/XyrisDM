...
public class CustomAction {
  ...
  public void execute(Player attacker, Player victim) {
    String formattedCommand, attackerName = (attacker != null) ? attacker.getName() : "?";
    String victimName = (victim != null) ? victim.getName() : "?";
    String formattedParameter = this.parameter.replace("%attacker%", attackerName).replace("%victim%", victimName);
    switch (this.type) {
      case CONSOLE_COMMAND:
        formattedCommand = formattedParameter.trim();
        formattedCommand = formattedCommand.replace("{attacker}", attackerName).replace("{victim}", victimName);
      ...
      case ATTACKER_COMMAND:
        if (attacker != null) {
          ...
          formattedAttackerCommand = formattedAttackerCommand.replace("{attacker}", attackerName).replace("{victim}", victimName);
      ...
      case VICTIM_COMMAND:
        if (victim != null) {
          ...
          formattedVictimCommand = formattedVictimCommand.replace("{attacker}", attackerName).replace("{victim}", victimName);
